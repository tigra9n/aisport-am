...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // article page being a dead end. Only shown for real stored articles;
  // demo/placeholder articles have no category to reliably match on.
  const related = stored
    ? (await getArticlesByCategory(stored.category, 7)).filter((a) => a.slug !== slug).slice(0, 3)
    : [];
