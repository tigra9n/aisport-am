...
export default async function LivePage({searchParams}:{searchParams:Promise<{date?:string;day?:string}>}){
 ...
 <span className="page-kicker">Խաղային կենտրոն</span><h1 className="page-title">Live արդյունքներ</h1><p className="page-intro">Football-Data.org-ի live հաշիվներ։ Match Center-ը լրացվում է TheSportsDB-ի անվճար մանրամասն տվյալներով՝ երբ նույն խաղը այնտեղ հասանելի է։</p>
 ...
 <div className="live-page-grid"><section className="matchday-card"><div className="matchday-head"><span>{relativeLabel(selected.offset)} · {shortDate(selected.date)}</span><small className={live.unavailable?"data-source demo":"data-source real"}>{live.unavailable?"Football-Data.org անհասանելի է":"Football-Data.org · live"}</small></div>
 {live.matches.length?competitions.map(c=><section className="match-competition-group" key={c}><h2>{c}</h2>{live.matches.filter(m=>m.competition===c).map(m=><details className="match-expand" key={m.id}><summary className="match-row"><span className={m.isLive?"match-live-status live-beacon-status":""}>{m.status}</span><strong>{m.home}</strong><b className="score-big">{m.homeScore??"–"} : {m.awayScore??"–"}</b><strong>{m.away}</strong></summary><div className="match-expand-body"><span>{m.isLive?"🔴 Խաղը ընթացքի մեջ է · հաշիվը Football-Data.org-ից":"Խաղի մանրամասները"}</span><small>⚽ գոլեր · 🎯 ասիստներ · 🟨🟥 քարտեր · 🔄 փոխարինումներ · պենալտիներ · կազմեր՝ TheSportsDB Free-ից, եթե խաղը գտնվի</small><Link href={`/live?date=${selected.date}&match=${m.id}`} scroll={false}>Բացել Match Center →</Link></div></details>)}</section>):<div className="no-matches">{live.unavailable?"Football-Data.org-ից տվյալներ չեն ստացվել։":`${shortDate(selected.date)}-ին ընտրված մրցաշարերում հանդիպումներ չկան։`}</div>}</section><aside className="live-note"><h3>Երկու API, մեկ Match Center</h3><p>Հաշիվն ու live կարգավիճակը՝ Football-Data.org-ից։ Խաղի լրացուցիչ մանրամասները՝ TheSportsDB Free-ից։ Եթե երկրորդ աղբյուրը տվյալ չունի, ցույց ենք տալիս միայն հաստատված հասանելի տվյալները։</p></aside></div>
